...
/**
 * @file SStormProfilePainter.h
 * @brief Declares the vertical profile painter window.
 */
...
private:
    ...
    EVisibility GetClearCanvasButtonVisibility() const;
    bool CanClearCanvas() const;
    ...
    FReply OnClearCanvas();
    FReply OnRevert();
    FReply OnNewProfile();
    FReply OnSaveAs();
    FReply OnSave();
    bool CanSaveToCurrentAsset() const;
    FText GetSaveButtonTooltip() const;
    /**
     * Guarantees the tool is targeting a writable profile before a structural edit.
     *
     * Only the key timeline is read-only on a shipped profile: pixels and parameters
     * live in the tool's working set, so painting and parameterization never touch the
     * asset. The key list is the asset, though, so there is nothing to preview a key
     * change against. Offers to fork into the authoring root and retargets the tool at
     * the copy.
     *
     * @return true when the tool now targets a writable asset; false if the user
     *         declined or the fork failed, in which case the caller must not proceed.
     */
    bool EnsureEditableProfileAsset();
    FReply OnLoad();
    ...
    /** Republishes the active profile and redraws viewports without rebuilding details widgets. */
