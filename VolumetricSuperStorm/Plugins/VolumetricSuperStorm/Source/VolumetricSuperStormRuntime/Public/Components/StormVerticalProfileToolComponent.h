...
/**
 * @file StormVerticalProfileToolComponent.h
 * @brief Declares vertical profile authoring, baking, and edit history.
 */
 ...
 */
USTRUCT()
struct VOLUMETRICSUPERSTORMRUNTIME_API FStormProfileKeyRenderTargetSet
{
	GENERATED_BODY()
...
class VOLUMETRICSUPERSTORMRUNTIME_API UStormVerticalProfileToolComponent : public UActorComponent
{
	GENERATED_BODY()
...
public:
	...
	/**
	 * Returns whether the selected Top/Anvil surface contains paint above its last
	 * applicable reset. bAnvil selects Anvil when true and Top when false.
	 */
	bool HasPaintedProfile(bool bAnvil) const;
	/**
	 * Returns whether the selected Top/Anvil surface still holds anything a wipe
	 * would remove. False once Clear Canvas has emptied it, which is what stops the
	 * button appending fills that change nothing. Not the inverse of
	 * HasPaintedProfile: an emptied surface is painted but not clearable.
	 */
	bool HasClearableCanvas(bool bAnvil) const;
	/**
	 * Wipes the selected Top/Anvil surface to an empty canvas and records a
	 * target-scoped history operation. Returns false if no pass ran. To restore the
	 * parametric macro instead, enter Parameterize mode: that reseeds both surfaces.
	 */
	bool ClearCanvas(bool bAnvil);
...
private:
	...
	bool BuildTopTypeProfile(UTextureRenderTarget2D* TargetRT);
	bool BuildAnvilProfile();
	/**
	...
	bool StampBrushInto(
		...
		bool bOverwrite);
	/**
	 * Overwrites every texel of an explicit target with a uniform level. ClearCanvas
	 * forwards here for the live surfaces; replay forwards here for scratch ones.
	 */
	bool FillProfileSurface(UTextureRenderTarget2D* TargetRT, float Value);
	/**
...
private:
	...
	/**
	 * Appends one target-scoped surface fill after ClearCanvas has successfully
	 * issued its render pass. The level is the whole payload, so unlike a reseed
	 * this records no params snapshot.
	 */
	bool RecordSurfaceFill(bool bAnvil, float Value);
