...
/**
 * @file StormProfileAssetEditorUtils.cpp
 * @brief Implements save, load, and validation helpers for vertical profile assets.
 */
...
{
/**
 * Whether an authoring action may write to ProfileAsset.
 *
 * Guards sit inside these helpers rather than at their call sites: the painter
 * disables its Save button for shipped assets, but key add/duplicate/remove
 * mutate and dirty the asset before any save, so a later "Save All" would
 * persist edits to plugin defaults anyway.
 */
bool CanMutate(const UStormVerticalProfileAsset* ProfileAsset)
{
...
namespace VolumetricSuperStorm::ProfileEditor
{
