...
int32 SStormFlowMapLayerRibbon::OnPaint(const FPaintArgs&, const FGeometry& Geometry, const FSlateRect&, FSlateWindowElementList& Out, int32 LayerId, const FWidgetStyle&, bool) const
{
	...
	for (float SlabTop = 0.0f; SlabTop < Height; SlabTop += SlabHeight)
	{
		...
		const FVector3f Weights    = VolumetricSuperStorm::FlowMap::EvaluateLayerWeights(LocalYToHeight((SlabTop + SlabBottom) * 0.5f, Geometry), Heights);
...
{
	return VolumetricSuperStorm::FlowMap::SanitizeLayerHeights(LayerHeightsAttr.Get(FVector3f(0.15f, 0.50f, 0.85f)));
}
...
{
	const FVector3f Weights = VolumetricSuperStorm::FlowMap::EvaluateLayerWeights(Height01, GetHeights());
...
FVector3f SStormFlowMapLayerRibbon::ApplyKnotDrag(int32 KnotIndex, float DesiredHeight) const
{
	...
	return VolumetricSuperStorm::FlowMap::SanitizeLayerHeights(Heights);
}
...
FText SStormFlowMapLayerRibbon::GetHoverTooltip() const
{
	...
	const FVector3f                Weights = VolumetricSuperStorm::FlowMap::EvaluateLayerWeights(HoveredHeight, GetHeights());
