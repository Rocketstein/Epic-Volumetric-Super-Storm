...
namespace
{
	...
	FVector GetMotionSliceCenterAtHeight(const UStormMaterialBinderComponent* Binder, const FStormRenderData& RenderData, double OuterBrimRadius01, double MotionHeight01)
	{
		...
		const float Radius01 = static_cast<float>(RadiusAcrossOuterBrim01);
		const double MinimumHeight01 = FMath::Clamp(
			static_cast<double>(Shape.ShapeCurves.LayerBottom.GetRichCurveConst()->Eval(Radius01)),
			0.0,
			1.0 - 1.0e-4);
		const double MaximumHeight01 = FMath::Clamp(
			static_cast<double>(Shape.ShapeCurves.LayerTop.GetRichCurveConst()->Eval(Radius01)),
			MinimumHeight01 + 1.0e-4,
			1.0);
...
void FStormMotionComponentVisualizer::DrawVisualizationHUD(const UActorComponent* Component, const FViewport* Viewport, const FSceneView* View, FCanvas* Canvas)
{
	...
	{
		const VolumetricSuperStorm::Motion::FStormRingInfluenceRange InfluenceRange        = VolumetricSuperStorm::Motion::GetRingInfluenceRange(RenderData.Motion, RingIndex);
