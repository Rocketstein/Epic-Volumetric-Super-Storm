...
/**
 * @file StormVerticalProfileToolComponent.cpp
 * @brief Implements vertical profile authoring, baking, and edit history.
 */
...
namespace VolumetricSuperStorm
{
	static TAutoConsoleVariable<bool> CVarLogProfileParamChanges(
		TEXT("VolumetricSuperStorm.Profile.LogParamChanges"),
		false,
	...
	static TAutoConsoleVariable<int32> CVarProfileHistoryMaxDepth(
		TEXT("VolumetricSuperStorm.Profile.History.MaxDepth"),
		128,
	...
	static TAutoConsoleVariable<int32> CVarProfileHistoryMaxReplayCost(
		TEXT("VolumetricSuperStorm.Profile.History.MaxReplayCost"),
		1024,
	...
	static TAutoConsoleVariable<int32> CVarProfileHistoryMaxEditReplayCost(
		TEXT("VolumetricSuperStorm.Profile.History.MaxEditReplayCost"),
		256,
...
UStormVerticalProfileToolComponent::UStormVerticalProfileToolComponent()
{
	...
	static ConstructorHelpers::FObjectFinder<UCurveFloat> DefaultProfileCurve(
		TEXT("/VolumetricSuperStorm/VolumetricSuperStorm/VerticalProfile/Internal/"
...
{
	constexpr int32 Resolution = VolumetricSuperStorm::RenderTargetResolution::Profile;
	...
	float Alpha)
{
	...
	ENQUEUE_RENDER_COMMAND(BlendStormProfilesRT)(
		...
		(FRHICommandListImmediate& RHICmdList)
		{
			...
			FVolumetricSuperStormProfileBlendPassParameters PassParameters;
			...
			FVolumetricSuperStormShaderInterface::AddProfileBlendPass_RenderThread(
...
void UStormVerticalProfileToolComponent::NotifyProfileRenderDataChanged()
{
	...
	{
		StormActor->RequestRenderDataUpdate(EStormRenderUpdateScope::Profile);
	}
}
	...
	const FStormProfileParams& Params)
{
	...
	FVolumetricSuperStormBottomTypePassParameters PassParameters;
	...
	ENQUEUE_RENDER_COMMAND(BuildBottomTypeProfileRT)(
		[PassParameters, RTResource, FeatureLevel](FRHICommandListImmediate& RHICmdList)
		{
			...
			FVolumetricSuperStormShaderInterface::AddBottomTypeProfilePass_RenderThread(
				GraphBuilder, GlobalShaderMap, PassParameters, RDGTexture);
			GraphBuilder.Execute();
		});
}

bool UStormVerticalProfileToolComponent::BuildTopTypeProfile(UTextureRenderTarget2D* TargetRT)
{
	return BuildTopTypeProfile(TargetRT, StormProfileParams);
}

bool UStormVerticalProfileToolComponent::BuildTopTypeProfile(
	UTextureRenderTarget2D* TargetRT,
	const FStormProfileParams& Params)
{
	UWorld* World = GetWorld();
	...
	// The shader indexes TopHeightLUT by dispatch x, so the LUT length and the dispatch
	// extent must both come from the target this pass actually writes.
	const int32 Resolution = FMath::Max(TargetRT->SizeX, 1);
	TArray<float> LUT;
	if (!SampleVerticalProfileCurve(
		Params.VerticalProfileCurve,
		static_cast<uint16>(Resolution),
		LUT,
		0.0f,
		1.0f))
	{
		UE_LOG(LogVolumetricSuperStormRuntime, Error,
			TEXT("Top profile curve is unusable; profile rebuild skipped."));
		return false;
	}

	FVolumetricSuperStormTopTypePassParameters PassParameters;
	...
	ENQUEUE_RENDER_COMMAND(BuildTopTypeProfileRT)(
		[PassParameters, RTResource, FeatureLevel](FRHICommandListImmediate& RHICmdList)
		{
			...
			FVolumetricSuperStormShaderInterface::AddTopTypeProfilePass_RenderThread(
				GraphBuilder, GlobalShaderMap, PassParameters, RDGTexture);
			GraphBuilder.Execute();
		});
	return true;
...
bool UStormVerticalProfileToolComponent::BuildAnvilProfile()
{
	return BuildAnvilProfile(AnvilProfileRT, StormProfileParams);
}
	...
	const FStormProfileParams& Params)
{
	...
	// The shader indexes HeightLUT by dispatch x, so the LUT length and the dispatch
	// extent must both come from the target this pass actually writes.
	const int32 Resolution = FMath::Max(TargetRT->SizeX, 1);
	TArray<float> LUT;
	if (!SampleVerticalProfileCurve(
	...
	FVolumetricSuperStormAnvilProfilePassParameters PassParameters;
	PassParameters.Resolution = static_cast<uint32>(Resolution);
	...
	ENQUEUE_RENDER_COMMAND(BuildAnvilProfileRT)(
		[PassParameters, RTResource, FeatureLevel](FRHICommandListImmediate& RHICmdList)
		{
			...
			FVolumetricSuperStormShaderInterface::AddAnvilProfilePass_RenderThread(
				GraphBuilder, GlobalShaderMap, PassParameters, RDGTexture);
			GraphBuilder.Execute();
		});
	return true;
	...
	bool bPaintAnvil)
{
	...
	if (AVolumetricSuperStormActor* StormActor = Cast<AVolumetricSuperStormActor>(GetOwner()))
	{
		StormActor->RequestTextureContentUpdate(
			bPaintAnvil
	...
	bool bOverwrite)
{
	...
	if (GMaxRHIShaderPlatform != SP_PCD3D_SM6 ||
		!IsFeatureLevelSupported(GMaxRHIShaderPlatform, ERHIFeatureLevel::SM6) ||
		FeatureLevel < ERHIFeatureLevel::SM6)
	{
		UE_LOG(LogVolumetricSuperStormRuntime, Error, TEXT("Vertical-profile painting requires DirectX 12 with Shader Model 6."));
		return false;
	}

	if (!RHIIsTypedUAVLoadSupported(PF_FloatRGBA))
	{
		UE_LOG(LogVolumetricSuperStormRuntime, Error, TEXT("Vertical-profile painting requires PF_FloatRGBA typed UAV load support."));
		return false;
	}
	...
	FVolumetricSuperStormProfileBrushPassParameters PassParameters;
	PassParameters.Resolution    = static_cast<uint32>(TargetRT->SizeX);
	...
	ENQUEUE_RENDER_COMMAND(StampProfileBrushRT)(
		[PassParameters, PaintResource, FeatureLevel](FRHICommandListImmediate& RHICmdList)
		{
			...
			FVolumetricSuperStormShaderInterface::AddProfileBrushPass_RenderThread(
...
bool UStormVerticalProfileToolComponent::IsProfileHistoryCheckpointRequired() const
{
	...
	const int64 MaxDepth =
		VolumetricSuperStorm::CVarProfileHistoryMaxDepth.GetValueOnGameThread();
	const int64 MaxReplayCost =
		VolumetricSuperStorm::CVarProfileHistoryMaxReplayCost.GetValueOnGameThread();
	...
	TFunctionRef<bool()> EstablishUndoBoundary)
{
	...
	ValidateKeyRenderTargetOwnership();
	UE_LOG(LogVolumetricSuperStormRuntime, Display,
	...
	const FStormProfileParams& OriginParams)
{
	...
	if (History.Graph.OriginStateId.IsValid())
	{
		if (!History.ContainsState(KeyState->CurrentStateId))
		{
			...
			UE_LOG(LogVolumetricSuperStormRuntime, Warning,
...
void UStormVerticalProfileToolComponent::RecordOp(const FStormProfileOp& Op)
{
	...
	else
	{
		const int64 MaxEditReplayCost =
			VolumetricSuperStorm::CVarProfileHistoryMaxEditReplayCost.GetValueOnGameThread();
...
bool UStormVerticalProfileToolComponent::RecordSurfaceFill(
	bool bAnvil,
	float Value)
{
	if (GIsTransacting)
	...
	// The level replays the fill on its own, so this records no params payload and
	// leaves nothing for the payload sweep to reclaim.
	FStormProfileOp Op;
	Op.Type = EStormProfileOpType::FillSurface;
	Op.bTargetAnvil = bAnvil;
	Op.Value = Value;
	FStormProfileEdit Edit;
	Edit.Type = EStormProfileEditType::ClearCanvas;
	Edit.Ops.Add(Op);
	const FGuid NewStateId = AppendHistoryEdit(
		*History,
		*KeyState,
	...
	UTextureRenderTarget2D* OutAnvil)
{
	...
	for (const FStormReplayHistoryNode* Node : Path)
	{
		...
		for (const FStormProfileOp& Op : Edit->Ops)
		{
			...
			// Nothing records this any more, but histories saved before Clear Canvas
			// replaced it still carry it. Keep replaying it or those surfaces come
			// back wrong.
			case EStormProfileOpType::ReseedSurfaceFromParams:
				if (const FStormProfileParams* OpParams =
					...
					!OpParams ||
					!(Op.bTargetAnvil
						? BuildAnvilProfile(OpTarget, *OpParams)
						: BuildTopTypeProfile(OpTarget, *OpParams)))
				{
					return false;
				}
				break;
			...
			case EStormProfileOpType::FillSurface:
				if (!FillProfileSurface(OpTarget, Op.Value))
				{
					return false;
				}
				break;
...
bool UStormVerticalProfileToolComponent::VerifyActiveKeyReplay(float& OutMaxDelta)
{
	...
	const bool bCompared =
		VolumetricSuperStorm::CompareProfileRenderTargets(
...
static FAutoConsoleCommand GVerifyProfileReplayCommand(
	TEXT("VolumetricSuperStorm.Profile.VerifyReplay"),
			...
			if (History)
			{
				...
				for (const FStormReplayHistoryNode* Node : AppliedPath)
				{
					...
					for (const FStormProfileOp& Op : Edit->Ops)
					{
						...
						case EStormProfileOpType::FillSurface:      ++Fills;  break;
						default: break;
						}
					}
				}
...
bool UStormVerticalProfileToolComponent::FillProfileSurface(
	UTextureRenderTarget2D* TargetRT,
	float Value)
{
	// The brush already writes a flat value with no falloff on its hard-overwrite
	// path, so one dab centred on the canvas is the whole fill -- no dedicated
	// shader needed. The half-diagonal out to a corner is sqrt(0.5) ~= 0.708, so a
	// radius of 1 reaches every texel with room to spare; the dispatch covers the
	// full target whatever the radius, making the slack free.
	return StampBrushInto(
		TargetRT,
		FVector2D(0.5, 0.5),
		/*BrushRadiusUV*/ 1.f,
		/*Strength*/ 1.f,
...
{
	return HasPaintedProfile(false) || HasPaintedProfile(true);
}

bool UStormVerticalProfileToolComponent::HasPaintedProfile(bool bAnvil) const
{
	if (IsHistoryStrokeOpen())
	{
		for (const FStormProfileOp& Op : PendingStrokeEdit.Ops)
		{
			if (Op.bTargetAnvil == bAnvil &&
				(Op.Type == EStormProfileOpType::Stamp ||
					Op.Type == EStormProfileOpType::LoadFromTexture))
			{
				return true;
			}
		}
...
bool UStormVerticalProfileToolComponent::HasClearableCanvas(bool bAnvil) const
{
	// Mid-stroke the surface certainly holds something, but ClearCanvas refuses to
	// run until the stroke seals, so report it as unavailable rather than offering
	// a wipe that would be declined.
	if (IsHistoryStrokeOpen())
	{
		return false;
	}
	const FStormProfileKeyHistory* History =
		FindKeyHistory(ActiveKeyRenderTargetId);
	return History && History->HasClearableContent(
		GetCurrentHistoryStateId(ActiveKeyRenderTargetId),
		bAnvil);
}

bool UStormVerticalProfileToolComponent::ClearCanvas(bool bAnvil)
{
	if (IsHistoryStrokeOpen())
	{
		return false;
	}
	if (!HasClearableCanvas(bAnvil))
	{
		return false;
	}
	...
	UTextureRenderTarget2D* TargetRT = bAnvil
		? AnvilProfileRT.Get()
		: TopTypeProfileRT.Get();
	// Zero is what the brush's own erase path writes, so an emptied canvas and an
	// erased dab agree on what "nothing here" looks like.
	if (!FillProfileSurface(TargetRT, 0.f))
	{
		return false;
	}

	if (!RecordSurfaceFill(bAnvil, 0.f))
	{
		ensureMsgf(false,
			TEXT("Failed to record a cleared profile canvas after wiping it."));
		return false;
	}

	MarkProfileDirty();
	if (AVolumetricSuperStormActor* StormActor =
			Cast<AVolumetricSuperStormActor>(GetOwner()))
	{
		StormActor->RequestTextureContentUpdate(
			bAnvil
				? EStormTextureDirtyFlags::ProfileAnvil
...
bool UStormVerticalProfileToolComponent::RevertActiveKeyToSaved()
{
	...
	if (SavedState)
	{
		...
		ModifyEditHistory();
		const bool bReverted = AppendSavedAssetRestore(*SavedState);
		if (bReverted && !ProfileSequenceRuntime.bPreviewActive)
		{
			NotifyProfileRenderDataChanged();
		}
		return bReverted;
	...
	const FGuid RestoreStateId = AppendHistoryEdit(
		...
		*KeyState,
		RestoreEdit,
		OriginContentId);
	const bool bReverted = RestoreStateId.IsValid() &&
	...
	const FStormProfileSavedState& SavedState)
{
	...
	UTexture2D* TopSnapshot =
		VolumetricSuperStorm::SnapshotProfileHistoryTexture(
			TopProfile,
			EditUndoState);
