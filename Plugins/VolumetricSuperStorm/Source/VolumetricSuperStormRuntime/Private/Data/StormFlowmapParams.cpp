...
namespace VolumetricSuperStorm::FlowMap
{
	...
	FVector3f EvaluateLayerWeights(float HLocal, const FVector3f& LayerHeights)
	{
		...
		const float LowerToMiddleLinear = FMath::Clamp((H - Heights.X) / FMath::Max(Heights.Y - Heights.X, MinLayerHeightGap), 0.0f, 1.0f);
		const float MiddleToUpperLinear = FMath::Clamp((H - Heights.Y) / FMath::Max(Heights.Z - Heights.Y, MinLayerHeightGap), 0.0f, 1.0f);
		const float LowerToMiddle       = LowerToMiddleLinear * LowerToMiddleLinear * (3.0f - 2.0f * LowerToMiddleLinear);
		const float MiddleToUpper       = MiddleToUpperLinear * MiddleToUpperLinear * (3.0f - 2.0f * MiddleToUpperLinear);
