...
namespace VolumetricSuperStorm::FlowMapRenderTargetUtils
{
	...
	bool ClearSurface(UTextureRenderTarget2D* Target)
	{
		...
		ENQUEUE_RENDER_COMMAND(ClearStormFlowMapRT)(
			[Resource, Resolution](FRHICommandListImmediate& RHICmdList)
			{
				...
				FVolumetricSuperStormShaderInterface::AddFlowMapClearPass_RenderThread(GraphBuilder, GetGlobalShaderMap(GMaxRHIFeatureLevel), Resolution, Texture);
	...
	bool StampSurface(UTextureRenderTarget2D* Target, const FStormFlowMapPaintParameters& PaintParameters)
	{
		...
		{
			UE_LOG(LogVolumetricSuperStormRuntime, Error, TEXT("Flow-map painting requires DirectX 12 with Shader Model 6."));
			return false;
		}

		if (!RHIIsTypedUAVLoadSupported(PF_FloatRGBA))
		{
			UE_LOG(LogVolumetricSuperStormRuntime, Error, TEXT("Flow-map painting requires PF_FloatRGBA typed UAV load support."));
			return false;
		}
		...
		FVolumetricSuperStormFlowMapBrushPassParameters PassParameters;
		...
		ENQUEUE_RENDER_COMMAND(StampStormFlowMapBrushRT)(
			[Resource, PassParameters](FRHICommandListImmediate& RHICmdList)
			{
				...
				FVolumetricSuperStormShaderInterface::AddFlowMapBrushPass_RenderThread(GraphBuilder, GetGlobalShaderMap(GMaxRHIFeatureLevel), PassParameters, Texture);
				GraphBuilder.Execute();
			}
		);
		return true;
