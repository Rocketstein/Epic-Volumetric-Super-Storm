...
void AVolumetricSuperStormActor::SetLightningSequence(const FStormLightningSequenceSettings& InSequence)
{
	LightningSequence = InSequence;
	ConfigureLightningComponent();
}

void AVolumetricSuperStormActor::SetLightningRepeatDelay(const float InDelaySeconds)
{
	LightningRepeatDelaySeconds = FMath::Max(0.1f, InDelaySeconds);
	ConfigureLightningComponent();
}

void AVolumetricSuperStormActor::SetGroundStrikeProbability(const float InProbability)
{
