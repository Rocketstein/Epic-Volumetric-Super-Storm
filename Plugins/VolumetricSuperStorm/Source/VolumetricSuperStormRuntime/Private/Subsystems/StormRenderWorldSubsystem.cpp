...
DECLARE_STATS_GROUP(TEXT("VolumetricSuperStorm"), STATGROUP_VolumetricSuperStorm, STATCAT_Advanced);
DECLARE_CYCLE_STAT(TEXT("Subsystem Tick"), STAT_SSS_Tick, STATGROUP_VolumetricSuperStorm);
DECLARE_CYCLE_STAT(TEXT("Submit Render Data"), STAT_SSS_SubmitRenderData, STATGROUP_VolumetricSuperStorm);
DECLARE_CYCLE_STAT(TEXT("Build Shape RT (GT)"), STAT_SSS_BuildShapeRT, STATGROUP_VolumetricSuperStorm);
DECLARE_GPU_STAT_NAMED(SSS_ShapePass, TEXT("VolumetricSuperStorm ShapePass"));

static TAutoConsoleVariable<int32> CVarStats(
	TEXT("VolumetricSuperStorm.Stats"),
	0,
	TEXT("When non-zero, draws the VolumetricSuperStorm storm-state overlay (4-decimal timings + render data)."),
	ECVF_Default);
	...
	uint32& OutHash)
{
	...
	for (int32 Index = 0; Index < CurveSampleCount; ++Index)
	{
		...
		const float BodyCoverage = FMath::Clamp(
			Shape.ShapeCurves.BodyCoverage.GetRichCurveConst()->Eval(Radius01), 0.0f, 1.0f);
		const float AnvilFill = FMath::Clamp(
			Shape.ShapeCurves.AnvilFill.GetRichCurveConst()->Eval(Radius01), 0.0f, 1.0f);
		const float BottomSoftness = FMath::Clamp(
			Shape.ShapeCurves.BottomSoftness.GetRichCurveConst()->Eval(Radius01), 0.0f, 1.0f);
		const float TopAnvilProfileCoordinate = FMath::Clamp(
			Shape.ShapeCurves.TopAnvilProfileCoordinate.GetRichCurveConst()->Eval(Radius01),
			0.0f,
			1.0f);
		const float LayerBottom = FMath::Clamp(
			Shape.ShapeCurves.LayerBottom.GetRichCurveConst()->Eval(Radius01), 0.0f, 1.0f);
		const float LayerTop = FMath::Clamp(
			Shape.ShapeCurves.LayerTop.GetRichCurveConst()->Eval(Radius01), 0.0f, 1.0f);

		// Preserve the shader-facing channel layout while exposing each authored
		// value as a separately named float curve in Details panels.
		OutCoverageLUT.Emplace(BodyCoverage, 0.0f, 0.0f, AnvilFill);
		OutTypeLUT.Emplace(0.0f, BottomSoftness, TopAnvilProfileCoordinate, 0.0f);
		OutLayerHeightLUT.Emplace(LayerBottom, 0.0f, LayerTop, 0.0f);

		CombineShapeHash(OutHash, BodyCoverage);
		CombineShapeHash(OutHash, AnvilFill);
		CombineShapeHash(OutHash, BottomSoftness);
		CombineShapeHash(OutHash, TopAnvilProfileCoordinate);
		CombineShapeHash(OutHash, LayerBottom);
		CombineShapeHash(OutHash, LayerTop);
	}
}
...
void UStormRenderWorldSubsystem::Tick(float)
{
	...
	if (bShapeDirty)
	{
		const bool bShapeBuilt = BuildShapeRenderTargets();
		bShapeDirty = !bShapeBuilt;
		if (bShapeBuilt)
		{
			// Queue anchor copies after the shape bake so render-thread execution preserves source-write order.
			if (AVolumetricSuperStormActor* StormActor = RegisteredStorm.Get())
			{
				StormActor->RequestTextureContentUpdate(
					EStormTextureDirtyFlags::Shape | EStormTextureDirtyFlags::Shape2);
			}
		}
	}
}
...
{
	constexpr int32 Resolution = VolumetricSuperStorm::RenderTargetResolution::Shape;

	if (RenderTarget &&
		RenderTarget->SizeX == Resolution &&
	...
	if (!RenderTarget)
	{
		...
		if (!RenderTarget)
		{
			UE_LOG(LogVolumetricSuperStormRuntime, Error, TEXT("Failed to create %s."), RenderTargetName);
			return false;
		}
	}
	...
	RenderTarget->InitCustomFormat(Resolution, Resolution, PF_FloatRGBA, true);
...
bool UStormRenderWorldSubsystem::ValidateRenderTarget(const UTextureRenderTarget2D* RenderTarget, const TCHAR* RenderTargetName) const
{
	const bool bValid =
		...
		RenderTarget->SizeX == VolumetricSuperStorm::RenderTargetResolution::Shape &&
		RenderTarget->SizeY == VolumetricSuperStorm::RenderTargetResolution::Shape &&
		RenderTarget->GetFormat() == PF_FloatRGBA &&
		RenderTarget->Filter == TF_Bilinear;
...
bool UStormRenderWorldSubsystem::BuildShapeRenderTargets()
{
	...
	FVolumetricSuperStormShapePassParameters PassParameters;
	...
	ENQUEUE_RENDER_COMMAND(BuildStormShapeRT)(
		[PassParameters, RTResource, RTResource2, UpperFlowMapResource, FeatureLevel](FRHICommandListImmediate& RHICmdList)
		{
			...
			{
				RDG_EVENT_SCOPE_STAT(GraphBuilder, SSS_ShapePass, "VolumetricSuperStorm ShapePass");
				FVolumetricSuperStormShaderInterface::AddShapePass_RenderThread(
					GraphBuilder,
					GlobalShaderMap,
					PassParameters,
					RDGUpperFlowMap,
...
void UStormRenderWorldSubsystem::DrawStatsOverlay(UCanvas* Canvas, APlayerController* )
{
	...
	Line(FLinearColor(0.4f, 0.8f, 1.0f), TEXT("VolumetricSuperStorm Stats"));
	...
	Line(FLinearColor::White, FString::Printf(
		...
		bShapeDirty ? 1 : 0,
		VolumetricSuperStorm::RenderTargetResolution::Shape,
