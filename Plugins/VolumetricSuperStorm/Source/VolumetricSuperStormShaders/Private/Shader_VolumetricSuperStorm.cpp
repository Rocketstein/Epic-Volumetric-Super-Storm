...
/**
 * @file Shader_VolumetricSuperStorm.cpp
 * @brief Implements render-graph compute passes used by the storm renderer.
 */

#include "Shader_VolumetricSuperStorm.h"
...
DEFINE_LOG_CATEGORY_STATIC(LogVolumetricSuperStormShaders, Log, All);

namespace
{
	bool IsVolumetricSuperStormShaderPlatformSupported(EShaderPlatform Platform)
	{
		return Platform == SP_PCD3D_SM6 &&
			IsFeatureLevelSupported(Platform, ERHIFeatureLevel::SM6);
	}

	bool ValidateVolumetricSuperStormShaderRuntimeSupport(const TCHAR* PassName, bool bRequiresFloatRGBATypedUAVLoad = false)
	{
		if (!IsVolumetricSuperStormShaderPlatformSupported(GMaxRHIShaderPlatform))
		{
			UE_LOG(
				LogVolumetricSuperStormShaders,
				Error,
				TEXT("%s requires the DirectX 12 SM6 shader platform; the active shader platform is %d."),
				PassName,
				static_cast<int32>(GMaxRHIShaderPlatform));
			return false;
		}

		if (bRequiresFloatRGBATypedUAVLoad && !RHIIsTypedUAVLoadSupported(PF_FloatRGBA))
		{
			UE_LOG(
				LogVolumetricSuperStormShaders,
				Error,
				TEXT("%s requires PF_FloatRGBA typed UAV loads, which are not supported by the active DirectX 12 RHI."),
				PassName);
			return false;
		}
...
{
	DECLARE_EXPORTED_SHADER_TYPE(FStormShapeCS, Global, VOLUMETRICSUPERSTORMSHADERS_API);
...
public:
	...
	{
		return IsVolumetricSuperStormShaderPlatformSupported(Parameters.Platform);
	}
...
{
	DECLARE_EXPORTED_SHADER_TYPE(FStormBottomTypeCS, Global, VOLUMETRICSUPERSTORMSHADERS_API);
...
{
	DECLARE_EXPORTED_SHADER_TYPE(FStormTopTypeCS, Global, VOLUMETRICSUPERSTORMSHADERS_API);
...
{
	DECLARE_EXPORTED_SHADER_TYPE(FStormAnvilProfileCS, Global, VOLUMETRICSUPERSTORMSHADERS_API);
...
{
	DECLARE_EXPORTED_SHADER_TYPE(FStormProfileBrushCS, Global, VOLUMETRICSUPERSTORMSHADERS_API);
...
{
	DECLARE_EXPORTED_SHADER_TYPE(FStormProfileBlendCS, Global, VOLUMETRICSUPERSTORMSHADERS_API);
...
{
	DECLARE_EXPORTED_SHADER_TYPE(FStormFlowMapClearCS, Global, VOLUMETRICSUPERSTORMSHADERS_API);
...
{
	DECLARE_EXPORTED_SHADER_TYPE(FStormFlowMapBrushCS, Global, VOLUMETRICSUPERSTORMSHADERS_API);
...
IMPLEMENT_GLOBAL_SHADER(FStormShapeCS, "/VolumetricSuperStormShaders/Private/StormControlCS.usf", "BuildStormShapeRT", SF_Compute);
IMPLEMENT_GLOBAL_SHADER(FStormBottomTypeCS, "/VolumetricSuperStormShaders/Private/StormProfileCS.usf", "BuildBottomTypeProfileRT", SF_Compute);
IMPLEMENT_GLOBAL_SHADER(FStormTopTypeCS, "/VolumetricSuperStormShaders/Private/StormProfileCS.usf", "BuildTopTypeProfileRT", SF_Compute);
IMPLEMENT_GLOBAL_SHADER(FStormProfileBrushCS, "/VolumetricSuperStormShaders/Private/StormProfileCS.usf", "StampProfileBrushRT", SF_Compute);
IMPLEMENT_GLOBAL_SHADER(FStormProfileBlendCS, "/VolumetricSuperStormShaders/Private/StormProfileCS.usf", "BlendProfilesRT", SF_Compute);
IMPLEMENT_GLOBAL_SHADER(FStormFlowMapClearCS, "/VolumetricSuperStormShaders/Private/StormFlowMapPaintCS.usf", "ClearFlowMapRT", SF_Compute);
IMPLEMENT_GLOBAL_SHADER(FStormAnvilProfileCS, "/VolumetricSuperStormShaders/Private/StormProfileCS.usf", "BuildAnvilProfileRT", SF_Compute);
IMPLEMENT_GLOBAL_SHADER(FStormFlowMapBrushCS, "/VolumetricSuperStormShaders/Private/StormFlowMapPaintCS.usf", "StampFlowMapBrushRT", SF_Compute);

void FVolumetricSuperStormShaderInterface::AddShapePass_RenderThread(FRDGBuilder& GraphBuilder, FGlobalShaderMap* InShaderMap, const FVolumetricSuperStormShapePassParameters& InPassParameters, FRDGTextureRef InUpperFlowMapTexture, FRDGTextureRef InTextureRef, FRDGTextureRef InTextureRef2)
{
	ensure(IsInRenderingThread());
	if (!ValidateVolumetricSuperStormShaderRuntimeSupport(TEXT("BuildStormShapeRT")))
	{
		return;
	}
...
void FVolumetricSuperStormShaderInterface::AddBottomTypeProfilePass_RenderThread(FRDGBuilder& GraphBuilder, FGlobalShaderMap* InShaderMap, const FVolumetricSuperStormBottomTypePassParameters& InPassParameters, FRDGTextureRef InTextureRef)
{
	ensure(IsInRenderingThread());
	if (!ValidateVolumetricSuperStormShaderRuntimeSupport(TEXT("BuildBottomTypeProfileRT")))
	{
		return;
	}
...
void FVolumetricSuperStormShaderInterface::AddTopTypeProfilePass_RenderThread(FRDGBuilder& GraphBuilder, FGlobalShaderMap* InShaderMap, const FVolumetricSuperStormTopTypePassParameters& InPassParameters, FRDGTextureRef InTextureRef)
{
	ensure(IsInRenderingThread());
	if (!ValidateVolumetricSuperStormShaderRuntimeSupport(TEXT("BuildTopTypeProfileRT")))
	{
		return;
	}
...
void FVolumetricSuperStormShaderInterface::AddAnvilProfilePass_RenderThread(FRDGBuilder& GraphBuilder, FGlobalShaderMap* InShaderMap, const FVolumetricSuperStormAnvilProfilePassParameters& InPassParameters, FRDGTextureRef InTextureRef)
{
	ensure(IsInRenderingThread());
	if (!ValidateVolumetricSuperStormShaderRuntimeSupport(TEXT("BuildAnvilProfileRT")))
	{
		return;
	}
...
void FVolumetricSuperStormShaderInterface::AddProfileBrushPass_RenderThread(FRDGBuilder& GraphBuilder, FGlobalShaderMap* InShaderMap, const FVolumetricSuperStormProfileBrushPassParameters& InPassParameters, FRDGTextureRef InPaintedTopTexture)
{
	ensure(IsInRenderingThread());
	if (!ValidateVolumetricSuperStormShaderRuntimeSupport(TEXT("StampProfileBrushRT"), true))
	{
		return;
	}
...
void FVolumetricSuperStormShaderInterface::AddProfileBlendPass_RenderThread(FRDGBuilder& GraphBuilder, FGlobalShaderMap* InShaderMap, const FVolumetricSuperStormProfileBlendPassParameters& InPassParameters, FRDGTextureRef InProfileA, FRDGTextureRef InProfileB, FRDGTextureRef OutBlendedProfileTexture)
{
	ensure(IsInRenderingThread());
	if (!ValidateVolumetricSuperStormShaderRuntimeSupport(TEXT("BlendStormProfilesRT")))
	{
		return;
	}
...
void FVolumetricSuperStormShaderInterface::AddFlowMapClearPass_RenderThread(FRDGBuilder& GraphBuilder, FGlobalShaderMap* InShaderMap, uint32 InResolution, FRDGTextureRef InOutFlowMap)
{
	ensure(IsInRenderingThread());
	if (!ValidateVolumetricSuperStormShaderRuntimeSupport(TEXT("ClearFlowMapRT")))
	{
		return;
	}
...
void FVolumetricSuperStormShaderInterface::AddFlowMapBrushPass_RenderThread(FRDGBuilder& GraphBuilder, FGlobalShaderMap* InShaderMap, const FVolumetricSuperStormFlowMapBrushPassParameters& InPassParameters, FRDGTextureRef InOutFlowMap)
{
	ensure(IsInRenderingThread());
	if (!ValidateVolumetricSuperStormShaderRuntimeSupport(TEXT("StampFlowMapBrushRT"), true))
	{
		return;
	}
