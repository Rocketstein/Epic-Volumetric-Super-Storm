...
namespace
{
    const FString DefaultVerticalProfilePath(TEXT("/SavageSuperStorm/VerticalProfile"));
    ...
    // Modal picker for the Load button. Uses a class-filtered asset picker rather than the
    // stock CreateModalOpenAssetDialog: because UStormVerticalProfileAsset is a dedicated
    // class, filtering to it surfaces exactly the profiles regardless of folder, which
    // sidesteps the stock dialog's unreliable plugin-folder default. Returns the picked
    // asset, or nullptr if cancelled.
    UStormVerticalProfileAsset* PickProfileAssetModal()
    {
        FAssetData SelectedAsset;
        bool bConfirmed = false;

        TSharedRef<SWindow> Window = SNew(SWindow)
            .Title(INVTEXT("Load Vertical Profile"))
            .ClientSize(FVector2D(520.f, 520.f))
            .SupportsMinimize(false)
            .SupportsMaximize(false);
        TWeakPtr<SWindow> WeakWindow = Window;

        FAssetPickerConfig PickerConfig;
        PickerConfig.Filter.ClassPaths.Add(UStormVerticalProfileAsset::StaticClass()->GetClassPathName());
        ...
        return Cast<UStormVerticalProfileAsset>(SelectedAsset.GetAsset());
    }
    ...
    ChildSlot
        [
                    ...
                    [
                        ...
                        [
                            ...
                            [
                                ...
                                .IsEnabled_Lambda([this]()
                                {
                                    return GetEditedProfileAsset() != nullptr;
                                })
            ...
            [
                ...
                [
                    SNew(SButton)
                    .Visibility(this, &SStormProfilePainter::GetClearPaintButtonVisibility)
                    .Text(INVTEXT("Clear Paint Layer"))
                    .ToolTipText(INVTEXT("Replace the painted Top and Anvil surfaces with the current parametric profile. Undo restores the paint."))
                    .IsEnabled(this, &SStormProfilePainter::CanClearPaintLayer)
                    .OnClicked(this, &SStormProfilePainter::OnClearPaintLayer)
                ]
                ...
                [
                    SNew(SButton)
                    .Text(INVTEXT("Save"))
                    .ToolTipText(INVTEXT("Save into the targeted Vertical Profile asset (falls back to Save As... when none is set)."))
                    .IsEnabled(this, &SStormProfilePainter::HasTarget)
...
void SStormProfilePainter::HandleUndoRedo(bool bSuccess)
{
    ...
    if (!StaleKeys.IsEmpty())
    {
        ...
        if (Asset &&
            ...
            SavageSuperStorm::ProfileEditor::FindKeyIndexById(
                Asset,
    ...
    const float InDeltaTime)
{
    ...
    // Editor-world components may not tick while only a viewport is open. Drive
    // playback from Slate so the painter preview and viewport stay live.
    if (!Tool->IsProfileSequencePlaying())
    {
        return;
    }
    Tool->AdvanceProfileSequencePreview(InDeltaTime);
...
void SStormProfilePainter::SetTarget(UStormVerticalProfileToolComponent* InTool)
{
    ...
    // editor viewport. RebuildRenderData re-reads the (editor) getter -> live RT, and
    // NotifyRenderDataUpdated pushes it to the material binder. Once bound, the RT is a
    ...
    if (InTool)
    {
        ...
        {
            Actor->RefreshProfileRenderData();
        }
    }
...
{
    return SavageSuperStorm::ProfileEditor::FindKeyIndexById(
        GetEditedProfileAsset(),
        SelectedKeyId);
}
...
float SStormProfilePainter::GetBrushRadiusUV() const
{
    ...
    return static_cast<float>(BrushTexels) / FMath::Max(1, Tool->StormProfileParams.StormProfileResolution);
}
...
EVisibility SStormProfilePainter::GetClearPaintButtonVisibility() const
{
    return IsPaintMode() ? EVisibility::Visible : EVisibility::Collapsed;
}

bool SStormProfilePainter::CanClearPaintLayer() const
{
    const UStormVerticalProfileToolComponent* Tool = GetTool();
    return Tool && IsPaintMode() && Tool->HasPaintedProfiles();
}
...
FReply SStormProfilePainter::OnClearPaintLayer()
{
    if (UStormVerticalProfileToolComponent* Tool = GetTool();
        Tool && CanClearPaintLayer())
    {
        ...
        const FScopedTransaction Transaction(INVTEXT("Clear Profile Paint Layer"));
        Tool->RebuildVerticalProfiles();
    }
    return FReply::Handled();
}
...
void SStormProfilePainter::RefreshTimelineKeys()
{
    ...
    if (SavageSuperStorm::ProfileEditor::FindKeyIndexById(
            Asset,
            SelectedKeyId) == INDEX_NONE &&
        !Asset->Keys.IsEmpty())
    {
...
bool SStormProfilePainter::CommitAllKeys()
{
    ...
    if (!SavageSuperStorm::ProfileEditor::CommitToolToAllKeys(
            Tool,
            Asset,
            SelectedKeyId))
    {
    ...
    TOptional<float> RequestedTime)
{
    ...
    UStormVerticalProfileAsset* Asset =
        GetEditedProfileAsset();
    const int32 SourceIndex = GetSelectedKeyIndex();
    if (!Tool || !Asset ||
    ...
    const bool bDuplicated = RequestedTime.IsSet()
        ? SavageSuperStorm::ProfileEditor::DuplicateKeyAtTime(
            Asset,
            SourceIndex,
            RequestedTime.GetValue(),
            NewKeyId)
        : SavageSuperStorm::ProfileEditor::DuplicateKeyAfter(
            Asset,
            SourceIndex,
            NewKeyId);
    if (!bDuplicated)
...
FReply SStormProfilePainter::OnDeleteSelectedKey()
{
    ...
    if (!SavageSuperStorm::ProfileEditor::RemoveKey(
            Asset,
            KeyIndex))
    {
        return FReply::Handled();
...
FReply SStormProfilePainter::OnSaveAs()
{
    ...
    // SDlgPickAssetPath sets bAddDefaultPath = true on its path picker, so the plugin folder
    // is reliably created/selected as the default. The stock CreateModalSaveAssetDialog does
    // not, and silently falls back to /Game for a not-yet-populated plugin folder. Note the
    // dialog rejects an existing asset name, so Save As always creates a NEW profile; use
    // Save to overwrite the currently targeted asset.
    TSharedRef<SDlgPickAssetPath> PickAssetPathDlg = SNew(SDlgPickAssetPath)
        .Title(INVTEXT("Save Vertical Profile As"))
        .DefaultAssetPath(FText::FromString(DefaultVerticalProfilePath + TEXT("/VP_StormVerticalProfile")));

    if (PickAssetPathDlg->ShowModal() != EAppReturnType::Ok)
    {
        return FReply::Handled();
    }

    const FString ProfilePackageName = PickAssetPathDlg->GetFullAssetPath().ToString();
    const FString ProfileAssetName = PickAssetPathDlg->GetAssetName().ToString();
    if (!FPackageName::IsValidLongPackageName(ProfilePackageName) || ProfileAssetName.IsEmpty())
    {
        UE_LOG(LogTemp, Warning, TEXT("Invalid vertical profile asset path: %s"), *ProfilePackageName);
        return FReply::Handled();
    }
    ...
    if (ProfileAsset && SourceAsset)
    {
        if (!SavageSuperStorm::ProfileEditor::CopyKeysToAsset(
                SourceAsset,
                ProfileAsset))
        {
            UE_LOG(LogTemp, Warning,
    ...
    if (ProfileAsset &&
        !ProfileAsset->Keys.IsValidIndex(
            DestinationKeyIndex))
    {
        DestinationKeyIndex = 0;
    }
...
FReply SStormProfilePainter::OnLoad()
{
    ...
    UStormVerticalProfileAsset* ProfileAsset = PickProfileAssetModal();
    if (!ProfileAsset)
    {
