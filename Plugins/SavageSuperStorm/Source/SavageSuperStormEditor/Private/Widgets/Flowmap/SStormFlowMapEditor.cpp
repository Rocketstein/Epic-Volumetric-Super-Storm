	...
	TSharedRef<SWidget> PaintPanel = SNew(SBorder).BorderImage(FAppStyle::GetBrush(TEXT("ToolPanel.GroupBorder"))).Padding(12.0f)
		[
			...
			[
				...
				+ SHorizontalBox::Slot().AutoWidth().Padding(0.0f, 0.0f, 12.0f, 0.0f)
				[
					SNew(SSpinBox<float>).MinValue(1.0f).MaxValue(static_cast<float>(SavageSuperStorm::FlowMap::Resolution)).Value(BrushRadiusTexels).OnValueChanged(this, &SStormFlowMapEditor::SetBrushRadius)
				]
				+ SHorizontalBox::Slot().AutoWidth().VAlign(VAlign_Center).Padding(0.0f, 0.0f, 4.0f, 0.0f)
				[
			...
			[
				...
				+ SHorizontalBox::Slot().AutoWidth().Padding(0.0f, 0.0f, 6.0f, 0.0f)
				[
					SNew(SButton).Text(this, &SStormFlowMapEditor::GetSaveButtonText).ToolTipText(LOCTEXT("SaveTooltip", "Bake the component's working flow maps into its targeted asset. Opens Save As when no asset is targeted.")).OnClicked(this, &SStormFlowMapEditor::SaveAsset)
				]
				+ SHorizontalBox::Slot().AutoWidth().Padding(0.0f, 0.0f, 6.0f, 0.0f)
				[
