...
class SAVAGESUPERSTORMRUNTIME_API AVolumetricSuperStormActor : public AActor
{
	GENERATED_BODY()

public:
	AVolumetricSuperStormActor();
	...
	/** @brief Uploads profile texture changes without rebuilding the shape field. */
	void RefreshProfileRenderData();
...
private:
	...
	enum class EStormRenderUpdateType : uint8
	{
		Frame,
		Material,
		Full
	...
	void  RebuildRenderDataInternal(EStormRenderUpdateType UpdateType);
	void  NotifyRenderDataUpdatedInternal(EStormRenderUpdateType UpdateType);
