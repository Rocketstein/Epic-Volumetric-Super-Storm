...
namespace SavageSuperStorm
{
	static TAutoConsoleVariable<int32> CVarFlowMapHistoryMaxDepth(
		TEXT("SavageStorm.FlowMap.History.MaxDepth"),
		128,
	...
	static TAutoConsoleVariable<int32> CVarFlowMapHistoryMaxReplayCost(
		TEXT("SavageStorm.FlowMap.History.MaxReplayCost"),
		1024,
	...
	static TAutoConsoleVariable<int32> CVarFlowMapHistoryMaxEditReplayCost(
		TEXT("SavageStorm.FlowMap.History.MaxEditReplayCost"),
		256,
...
namespace
{
	...
	{
		Params.Resolution            = FlowMap::Resolution;
		...
		const FStormFlowmapParams& B)
	{
		return A.Resolution == B.Resolution &&
			A.LowerLayerHeight == B.LowerLayerHeight &&
...
{
	if (InParams.Resolution != FlowMap::Resolution)
	{
		// Resolution identifies the pixel document rather than an editable property.
		// Unsupported changes therefore discard the current document and history.
	...
	const int32 Resolution                = FlowMap::Resolution;
...
bool UStormFlowMapComponent::EnsureFlowMapsInitialized()
{
	...
	const bool bHasUsableTargets = LowerFlowMapRT && MiddleFlowMapRT && UpperFlowMapRT && LowerFlowMapRT->GetOuter() == this && MiddleFlowMapRT->GetOuter() == this && UpperFlowMapRT->GetOuter() == this && LowerFlowMapRT->SizeX == FlowMap::Resolution && LowerFlowMapRT->SizeY == FlowMap::Resolution && MiddleFlowMapRT->SizeX == FlowMap::Resolution && MiddleFlowMapRT->SizeY == FlowMap::Resolution && UpperFlowMapRT->SizeX == FlowMap::Resolution && UpperFlowMapRT->SizeY == FlowMap::Resolution;
	...
	bool bCurrentStateIsSaved)
{
	...
	const bool bHasOrigins =
		...
		HistoryOriginLowerRT->SizeX == FlowMap::Resolution &&
		HistoryOriginLowerRT->SizeY == FlowMap::Resolution &&
		HistoryOriginMiddleRT->SizeX == FlowMap::Resolution &&
		HistoryOriginMiddleRT->SizeY == FlowMap::Resolution &&
		HistoryOriginUpperRT->SizeX == FlowMap::Resolution &&
		HistoryOriginUpperRT->SizeY == FlowMap::Resolution;
	...
	HistoryOriginLowerRT = FlowMapRenderTargetUtils::CreateSurface(
		this, TEXT("RT_StormFlowMapHistoryOriginLower"), FlowMap::Resolution);
	HistoryOriginMiddleRT = FlowMapRenderTargetUtils::CreateSurface(
		this, TEXT("RT_StormFlowMapHistoryOriginMiddle"), FlowMap::Resolution);
	HistoryOriginUpperRT = FlowMapRenderTargetUtils::CreateSurface(
		this, TEXT("RT_StormFlowMapHistoryOriginUpper"), FlowMap::Resolution);
...
bool UStormFlowMapComponent::IsFlowMapHistoryCheckpointRequired() const
{
	...
	const int64 MaxDepth =
		SavageSuperStorm::CVarFlowMapHistoryMaxDepth.GetValueOnGameThread();
	const int64 MaxReplayCost =
		SavageSuperStorm::CVarFlowMapHistoryMaxReplayCost.GetValueOnGameThread();
	...
	TFunctionRef<bool()> EstablishUndoBoundary)
{
	...
	UTextureRenderTarget2D* NewOriginLower =
		FlowMapRenderTargetUtils::CreateSurface(
			this, TEXT("RT_StormFlowMapCheckpointLower"), FlowMap::Resolution);
	UTextureRenderTarget2D* NewOriginMiddle =
		FlowMapRenderTargetUtils::CreateSurface(
			this, TEXT("RT_StormFlowMapCheckpointMiddle"), FlowMap::Resolution);
	UTextureRenderTarget2D* NewOriginUpper =
		FlowMapRenderTargetUtils::CreateSurface(
			this, TEXT("RT_StormFlowMapCheckpointUpper"), FlowMap::Resolution);
...
void UStormFlowMapComponent::RecordOp(const FStormFlowMapOp& Op)
{
	...
	if (IsHistoryStrokeOpen())
	{
		const int64 MaxEditReplayCost =
			SavageSuperStorm::CVarFlowMapHistoryMaxEditReplayCost.GetValueOnGameThread();
