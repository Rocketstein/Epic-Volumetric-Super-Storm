...
void UStormLightningComponent::TriggerGroundStrike()
{
	...
	if (bUsedFallbackPlane && !bGroundFallbackReported)
	{
		...
		UE_LOG(LogStormLightning, Log, TEXT( "No queryable ground was loaded below %s. Using the Storm " "Actor-relative fallback plane at Z=%.1f; the Actor remains " "freely movable and no world-origin placement is required."), *GetPathName(), GroundWorld.Z);
	}
...
{
	return SequenceSettings.MaterialState.CenterN;
}
...
{
	const AActor* GroundReference = StormOwner.IsValid() ? static_cast<const AActor*>(StormOwner.Get()) : GetOwner();
	const double  AuthoredGroundZ = GroundReference ? GroundReference->GetActorLocation().Z : 0.0;
	...
	Result.Z       = FMath::Clamp(AuthoredGroundZ, LowestAllowedZ, HighestAllowedZ);
	return Result;
}
...
void UStormLightningComponent::ApplyLightningMaterialState()
{
	...
	Binder->ApplyLightningMaterialState(SequenceSettings.MaterialState);
}
