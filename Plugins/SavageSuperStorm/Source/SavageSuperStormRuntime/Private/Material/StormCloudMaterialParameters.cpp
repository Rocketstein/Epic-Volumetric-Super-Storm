...
namespace SavageSuperStorm::CloudMaterialParams
{
	...
	bool HasStormMaterialContract(const UMaterialInterface* Material)
	{
		...
		const FName RequiredScalars[]  = { FlowMapEnabled, StormCoordinateScale, UndersideVisibility, LifecycleControl, AnvilStrength, AnvilDepth01, AnvilAnchorHeight01, RotationSign, AnvilTwistRadians, AnvilTwistOrigin, OuterBrimRadiusScale, LFNoiseWorldSize, HFNoiseWorldSize, CurlNoiseWorldSize, LFUnitsPerBodyRadius, HFUnitsPerBodyRadius, CurlUnitsPerBodyRadius, DensityMultiplier, DensityGamma, HFStrength, StormTimeSeconds, LightningPulse, LightningCorePeak, LightningFillIntensity, LightningLeakIntensity };
		const FName RequiredVectors[]  = { FlowMapLayerHeights, FlowMapUVWStrength, StormCenterRadius, StormCenter, StormExtent, StormBaseCloudColor, StormReferenceCenter, BrimEmissiveColor, MDRBoundaries0, MDRBoundaries1, MDRSpeeds0, MDRSpeeds1, MDRPhases0, MDRPhases1, MDRSkews0, MDRSkews1, MDRControl0, MDRControl1, LightningCenter, LightningFlashExtent, LightningHaloExtent, LightningHotWhiteColor, LightningFillColor, LightningLeakColor };
