...
AVolumetricSuperStormActor::AVolumetricSuperStormActor()
{
...
void AVolumetricSuperStormActor::CreateEditorIconComponent()
{
	...
	if (!Plugin)
	{
		UE_LOG(LogSavageSuperStormRuntime, Warning, TEXT("Could not load the storm actor icon because the plugin was not found."));
		return;
	}
	...
	if (!IconTexture.IsValid())
	{
		UE_LOG(LogSavageSuperStormRuntime, Warning, TEXT("Could not load the storm actor icon from '%s'."), *IconPath);
		return;
	}
...
void AVolumetricSuperStormActor::OnConstruction(const FTransform& Transform)
{
	...
	SavageSuperStorm::Motion::SynchronizeRingArrays(MotionSettings);
...
void AVolumetricSuperStormActor::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	if (PropertyName == GET_MEMBER_NAME_CHECKED(FStormShapeCurves, CoverageStrength))
	{
		ShapeSettings.ShapeCurves.EnsureCoverageStrengthCurve();
	}
	else if (PropertyName == GET_MEMBER_NAME_CHECKED(FStormShapeCurves, TypeStrength))
	{
		ShapeSettings.ShapeCurves.EnsureTypeStrengthCurve();
	}
	else if (PropertyName == GET_MEMBER_NAME_CHECKED(FStormShapeCurves, LayerHeight))
	{
...
void AVolumetricSuperStormActor::Tick(float DeltaSeconds)
{
	...
	RebuildRenderDataInternal(EStormRenderUpdateType::Frame);
...
void AVolumetricSuperStormActor::ApplyPreset(UStormPresetDataAsset* InPreset)
{
	if (!InPreset)
	{
		UE_LOG(LogSavageSuperStormRuntime, Warning, TEXT("Storm '%s': Data asset is null. Failed to apply preset."), *GetName());
		return;
	}
	...
	if (VerticalProfileTool)
	{
		...
		{
			UE_LOG(LogSavageSuperStormRuntime, Warning, TEXT("Storm '%s': applied preset has an incomplete vertical profile asset."), *GetName());
		}
	}
...
void AVolumetricSuperStormActor::BuildPresetData(FStormPresetData& OutData) const
{
	...
	SavageSuperStorm::Motion::SynchronizeRingArrays(OutData.MotionSettings);
}
...
bool AVolumetricSuperStormActor::SetMotionRingEndRadius01ForEditor(int32 BoundaryIndex, float NewRadius01)
{
	...
	const float MinimumRadius01 = BoundaryIndex > 0 ? SanitizedMotion.RingEndRadii01[BoundaryIndex - 1] + SavageSuperStorm::Motion::MinRingWidth01 : SavageSuperStorm::Motion::MinRingWidth01;
	const float MaximumRadius01 = SanitizedMotion.RingEndRadii01.IsValidIndex(BoundaryIndex + 1) ? SanitizedMotion.RingEndRadii01[BoundaryIndex + 1] - SavageSuperStorm::Motion::MinRingWidth01 : FMath::Min(0.99f, SanitizedMotion.MotionRadiusScale - SavageSuperStorm::Motion::MinRingWidth01);
