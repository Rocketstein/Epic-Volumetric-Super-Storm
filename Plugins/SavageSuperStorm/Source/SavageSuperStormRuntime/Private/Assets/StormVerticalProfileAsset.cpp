	...
	FDataValidationContext& Context) const
{
	...
	for (int32 KeyIndex = 0; KeyIndex < Keys.Num(); ++KeyIndex)
	{
		...
		if (KeySizeX != INDEX_NONE)
		{
			if (KeySizeX != Key.Params.StormProfileResolution ||
				KeySizeY != Key.Params.StormProfileResolution)
			{
				AddError(FText::Format(
					LOCTEXT(
						"KeyResolutionMismatch",
						"Profile key {0}'s texture dimensions do not match its authored profile resolution."),
					KeyNumber));
			}
