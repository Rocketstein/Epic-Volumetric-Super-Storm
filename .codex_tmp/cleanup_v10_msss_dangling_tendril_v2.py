...
EXPECTED_CALL_INPUTS = [
    ...
    "BaseBottomType", "TopType", "StormStencil", "WorldPosition",
    "StormCenterRadius", "LFUVW", "HFUVW", "CurlUVW", "CurlTiling",
    "CurlDisplacementUVW", "StormLifecycleControl", "DensityGamma",
    "HFStrength",
]
...
direct_consumers = consumers_of(material, expressions, declaration)
if direct_consumers:
    raise RuntimeError(
        f"Refusing to delete directly consumed {STALE_NAME} declaration: "
        + ",".join(expr.get_name() for expr in direct_consumers)
    )
...
for expr in remaining:
    for property_name in (
        "name", "declaration_name", "parameter_name", "input_name", "desc",
    ):
        if STALE_NAME in str(prop(expr, property_name, "")):
            raise RuntimeError(
                f"Stale Tendril expression remains: {expr.get_name()}.{property_name}"
            )
